...
编辑服务时展开“运行环境”。自动模式从服务目录向上查找最近的 `.java-version`、`.nvmrc` 或 `.node-version`，没有版本文件时沿用 Shell 默认；手动选择的优先级最高。面板会显示解析后的版本、路径和来源。

自动检测本机 JDK、Homebrew Node.js 及 jenv/nvm 已安装版本，也支持自定义路径。数字版本不依赖管理器；`lts/*` 等管理器别名需要对应管理器能解析到已安装版本。指定版本缺失时阻止启动，不自动下载或静默换用其他版本。修改后重启该服务生效，不改变其他服务或终端的环境。
